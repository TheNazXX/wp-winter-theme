...
function add_gallery_metabox($post_type) {
	...
	if (in_array($post_type, $types)) {
		add_meta_box(
			'gallery-metabox',
			esc_html__('Slider Images','winter'),
  ...
  <tr>
    <td>
      <a class="gallery-add button" href="#"
        data-uploader-title="<?php echo esc_html_e('Add image(s) to gallery','fixa'); ?>"
        data-uploader-button-text="<?php echo esc_html_e('Add image(s)','fixa'); ?>">Add
        <?php echo esc_html_e('Add image(s)','fixa'); ?></a>

      <ul id="gallery-metabox-list" class="gallery_metabox_list">
        <?php if ($ids) : foreach ($ids as $key => $value) : $image = wp_get_attachment_image_src($value); ?>

        <li class="image_holder">
          <input type="hidden" name="ale_gallery_id[<?php echo esc_attr($key); ?>]"
            value="<?php echo esc_attr($value); ?>">
          <img class="image-preview" src="<?php echo esc_url($image[0]); ?>">
          <div class="buttons_manage">
            <a class="change-image button button-primary button-medium" href="#"
              data-uploader-title="<?php echo esc_html_e('Change image','fixa'); ?>"
              data-uploader-button-text="Change image">Change</a>
